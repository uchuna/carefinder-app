...
// Convert the fetched data to conform to the Hospital interface
const convertToHospitalFormat = (data: any[]): Hospital[] => {
    return data.map((item: any) => ({
        id: item.facility_id,
        name: item.facility_name,
        address: item.facility_address,
        phoneNumber: item.facility_phone_number,
        email: item.facility_email,
        state: item.state,
        provider: item.provider_type,
    }));
};

// Fetch and convert data asynchronously
const getHospitals = async (): Promise<Hospital[]> => {
    const data = await fetchHospitalData();
    return convertToHospitalFormat(data);
};
